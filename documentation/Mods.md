...
### Mod Orders
But how do we tell YAGDRSL which mods we want to apply? And when? For that we have mod *orders*. These are simply lists of mod names and we use them to define which mods apply to which actions and events as well as the order that the mods should be applied. Mod orders can be defined by function calls starting with SetModOrder___ which correspond to all the different combinations of events and actions.
```
